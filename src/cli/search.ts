...
if (exportFixture) {
  const fixture = buildSearchFixtureExport(result);
  if (!fixture) {
    console.log(
      JSON.stringify(
        {
          ok: false,
          error: "No first-ack violation found to export under this bounded search config.",
...
function printSearchReport(): void {
...
function parseArgs(args: string[]): { config: Partial<SearchConfig>; exportFixture: boolean } {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--seed" && next) {
      ...
      config.seeds = parseIntStrict(next, "seeds");
      index += 1;
    } else if (arg === "--nodes" && next) {
      config.nodeCount = parseIntStrict(next, "nodes");
      index += 1;
    } else if (arg === "--ops" && next) {
      config.operationCount = parseIntStrict(next, "ops");
      index += 1;
    } else if (arg === "--clients" && next) {
      config.clientCount = parseIntStrict(next, "clients");
      index += 1;
    } else if (arg === "--read-ratio" && next) {
      config.readRatio = parseFloatStrict(next, "read-ratio");
      index += 1;
    } else if (arg === "--chaos" && next) {
      config.partitionIntensity = parseFloatStrict(next, "chaos");
      index += 1;
    } else if ((arg === "--concurrency" || arg === "--overlap") && next) {
      config.concurrentIntensity = parseFloatStrict(next, "concurrency");
      index += 1;
    } else if (arg === "--protocol" && next) {
      config.protocol = parseProtocol(next);
      index += 1;
    } else if (arg === "--shrink") {
      config.shrink = true;
