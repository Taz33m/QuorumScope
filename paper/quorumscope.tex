...
\begin{itemize}
  ...
  curated fixture, default adversarial generated corpus, regression corpus, benchmark, and tiny
  exhaustive model reported here.
  \item \textbf{Claim:} \quorum{} produced zero linearizability violations in the declared bounded
  corpora under the current single-register and partition assumptions.
  \item \textbf{Claim:} \quorum{} pays an availability cost; unavailable operations are reported
  rather than hidden.
  \item \textbf{Non-claim:} this is not a proof of \quorum{} correctness for arbitrary systems,
  networks, schedules, or implementations.
  \item \textbf{Non-claim:} this is not a production database model, Raft/Paxos implementation,
  real network simulator, or durability model.
\end{itemize}
\end{minipage}}
\end{center}
...
\begin{center}
\begin{tabular}{L{0.38\linewidth} c L{0.38\linewidth}}
\textbf{Majority partition} & & \textbf{Minority partition} \\
\texttt{n3, n4, n5} & \Large$\parallel$ & \texttt{n1, n2} \\
\hline
write \texttt{op2}: commit \texttt{v1} & & read \texttt{op3}: returns \texttt{v0} under \firstack{} \\
response before \texttt{op3} starts & $\Rightarrow$ & stale successful read violates real-time order \\
\hline
\quorum{} has a majority here & & \quorum{} lacks a majority and returns unavailable \\
\end{tabular}
\end{center}
\end{minipage}}
\caption{Modeled split-brain stale-read witness. The violation is not that replicas disagree;
it is that \firstack{} reports a successful read of \texttt{v0} after a completed write to
\texttt{v1} must precede it in every legal single-register order.}
\end{figure}
...
All numbers in this section come from commands run on the current checkout before updating this
report. The reported product-result commit was \texttt{3e8b2b0}. The product verification command
completed successfully and includes tests, typecheck, production build, UI smoke, demo, corpus
replay, search comparison, exhaustive exploration, and product report.

\begin{table}[h]
\centering
\begin{tabular}{|L{0.31\linewidth}|L{0.6\linewidth}|}
\hline
...
\centering
\small
\begin{tabular}{|L{0.25\linewidth}|r|r|r|L{0.23\linewidth}|}
\hline
